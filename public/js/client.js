...
values.addEventListener("submit", (e) => {
  ...
  fetch(url).then((response) => {
    response.json().then((data) => {
      console.log(
        `${data.data[0].name} starts from ${data.data[0].startDate} and ends in ${data.data[0].endDate}. it has ${data.data[0].odi} odi , ${data.data[0].t20} t20 and ${data.data[0].test} test matches!🏏`
      );
      for (let i = 0; i < 5; i++) {
